...
export class CopyBtn extends Tonic {
    ...
    render () {
        return this.html`<button
            class="${('copy-btn' + this.props.class || '').trim()}"
        >
            ${this.state.hasCopied ? ('✅ \u00A0 copied!') : this.childNodes}
        </button>`
    }
}

export class CopyIconBtn extends Tonic {
    state = { hasCopied: false }
    ...
    render () {
        ...
        return this.html`<button
            ...${_props}
            class="${('copy-icon-btn ' +
                (this.state.hasCopied ? 'has-copied' : '') +
